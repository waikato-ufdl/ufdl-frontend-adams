...
/**
 * Allows the user to select jobs.
 *
 * @author FracPete (fracpete at waikato dot ac dot nz)
 */
public class UFDLJobChooserPanel
  extends AbstractUFDLSpreadSheetBasedSoftDeleteChooserPanel<Job> {
   ...
   */
  @Override
  protected Job loadObject(int pk) throws Exception {
    return m_Connection.getClient().jobs().load(pk);
  }
   ...
   */
  @Override
  protected Job[] newArray(int len) {
    return new Job[len];
  }
  ...
  protected AbstractUFDLObjectToSpreadSheetConversion getConversion() {
    return new UFDLJobToSpreadSheet();
  }
   ...
   */
  @Override
  protected int getPK(Job object) {
    return object.getPK();
  }
