...
import adams.data.conversion.UFDLNodeToSpreadSheet;
import com.github.fracpete.javautils.struct.Struct2;
import com.github.waikatoufdl.ufdl4j.action.Nodes.Node;

/**
 * Allows the user to select nodes.
 *
 * @author FracPete (fracpete at waikato dot ac dot nz)
 */
public class UFDLNodeChooserPanel
  extends AbstractUFDLSpreadSheetBasedChooserPanel<Node> {
   ...
   */
  @Override
  protected Node loadObject(int pk) throws Exception {
    return m_Connection.getClient().nodes().load(pk);
  }
   ...
   */
  @Override
  protected Node[] newArray(int len) {
    return new Node[len];
  }
  ...
  protected AbstractUFDLObjectToSpreadSheetConversion getConversion() {
    return new UFDLNodeToSpreadSheet();
  }
   ...
   */
  @Override
  protected int getPK(Node object) {
    return object.getPK();
  }
   ...
   */
  @Override
  protected Node[] getAvailableObjects() throws Exception {
    return m_Connection.getClient().nodes().list(m_Filter.generate(new MessageCollection())).toArray(new Node[0]);
  }
