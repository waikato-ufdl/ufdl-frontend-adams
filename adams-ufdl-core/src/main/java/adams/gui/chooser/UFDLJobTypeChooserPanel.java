...
/**
 * Allows the user to select job types.
 *
 * @author FracPete (fracpete at waikato dot ac dot nz)
 */
public class UFDLJobTypeChooserPanel
  extends AbstractUFDLSpreadSheetBasedChooserPanel<JobType> {
   ...
   */
  @Override
  protected Struct2<Integer, String> toStruct(JobType value) {
   ...
   */
  @Override
  protected JobType loadObject(String id) throws Exception {
    return m_Connection.getClient().jobTypes().load(id);
  }
   ...
   */
  @Override
  protected JobType loadObject(int pk) throws Exception {
    return m_Connection.getClient().jobTypes().load(pk);
  }
   ...
   */
  @Override
  protected JobType[] newArray(int len) {
    return new JobType[len];
  }
  ...
  protected AbstractUFDLObjectToSpreadSheetConversion getConversion() {
    return new UFDLJobTypeToSpreadSheet();
  }
   ...
   */
  @Override
  protected int getPK(JobType object) {
    return object.getPK();
  }
   ...
   */
  @Override
  protected JobType[] getAvailableObjects() throws Exception {
    return m_Connection.getClient().jobTypes().list(m_Filter.generate(new MessageCollection())).toArray(new JobType[0]);
  }
