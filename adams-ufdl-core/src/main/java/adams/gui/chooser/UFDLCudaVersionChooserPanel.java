...
/*
 * UFDLCudaVersionChooserPanel.java
...
import adams.data.conversion.UFDLCudaVersionToSpreadSheet;
import com.github.fracpete.javautils.struct.Struct2;
import com.github.waikatoufdl.ufdl4j.action.CudaVersions.CudaVersion;

/**
 * Allows the user to select cuda versions.
 *
 * @author FracPete (fracpete at waikato dot ac dot nz)
 */
public class UFDLCudaVersionChooserPanel
  extends AbstractUFDLSpreadSheetBasedChooserPanel<CudaVersion> {
   ...
   */
  @Override
  protected CudaVersion loadObject(String id) throws Exception {
    return m_Connection.getClient().cuda().load(id);
  }
   ...
   */
  @Override
  protected CudaVersion loadObject(int pk) throws Exception {
    return m_Connection.getClient().cuda().load(pk);
  }
   ...
   */
  @Override
  protected CudaVersion[] newArray(int len) {
    return new CudaVersion[len];
  }
  ...
  protected AbstractUFDLObjectToSpreadSheetConversion getConversion() {
    return new UFDLCudaVersionToSpreadSheet();
  }
   ...
   */
  @Override
  protected int getPK(CudaVersion object) {
    return object.getPK();
  }
  ...
  protected String getDialogTitle() {
    if (m_MultiSelection)
      return "Select CUDA versions";
    else
      return "Select CUDA version";
  }
}
