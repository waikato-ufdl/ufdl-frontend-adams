...
/**
 * Deletes the pretrained model either via PK or pretrained model name.
 *
 * @author FracPete (fracpete at waikato dot ac dot nz)
 */
public class DeletePretrainedModel
  ...
  public String globalInfo() {
    return "Deletes the pretrained model either via PK or pretrained model name.";
  }
  ...
  protected Object doTransform(PretrainedModel model, MessageCollection errors) {
    ...
    if (isLoggingEnabled())
      getLogger().info("Deleting pretrained model (hard=" + m_Hard + "): " + model);

    try {
      result = m_Client.pretrainedModels().delete(model, m_Hard);
    }
    catch (Exception e) {
      errors.add("Failed to delete pretrained model (hard=" + m_Hard + "): " + model, e);
    }
