...
/**
 * Updates the permissions of the specified team member(s) of the team passing through.
 *
...
public class UpdateTeamMember
  ...
  public String globalInfo() {
    return "Updates the permissions of the specified team member(s) of the team passing through.";
  }
  ...
  public String membersTipText() {
    return "The PKs of the members to update.";
  }
  ...
  protected Object doTransform(Team team, MessageCollection errors) {
    ...
    for (BaseInteger member: m_Members) {
      try {
        if (isLoggingEnabled())
          getLogger().info("Updating member " + member + " of team '" + team + "'");
        user = m_Client.users().load(member.intValue());
        if (!m_Client.teams().updateMembership(team, user, m_Permissions))
          errors.add("Failed to update member " + member + " of team '" + team + "': " + team);
      }
      catch (Exception e) {
        errors.add("Failed to update member " + member + " of team '" + team + "': " + team, e);
      }
    }
