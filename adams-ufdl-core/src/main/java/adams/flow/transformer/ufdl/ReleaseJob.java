...
public class ReleaseJob
  ...
  public String globalInfo() {
    return "Releases the job via PK.";
  }
  ...
  protected Object doTransform(Job job, MessageCollection errors) {
    ...
    if (isLoggingEnabled())
      getLogger().info("Releasing job: " + job);

    try {
      result = m_Client.jobs().release(job);
    }
    catch (Exception e) {
      errors.add("Failed to release job: " + job, e);
    }
