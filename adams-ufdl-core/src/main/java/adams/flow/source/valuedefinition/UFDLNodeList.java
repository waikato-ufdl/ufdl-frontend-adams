...
/**
 * For selecting a UFDL node.
 *
 * @author FracPete (fracpete at waikato dot ac dot nz)
 */
public class UFDLNodeList
  ...
  public String globalInfo() {
    return "For selecting a UFDL node.";
  }
  ...
  protected List<Struct2<Integer, String>> listItems() {
    ...
    try {
      for (Node node : m_Connection.getClient().nodes().list(m_Filter.generate(new MessageCollection())))
        result.add(new Struct2<>(node.getPK(), node.getIP()));
    }
    catch (Exception e) {
      getLogger().log(Level.SEVERE, "Failed to retrieve list of nodes!", e);
    }
