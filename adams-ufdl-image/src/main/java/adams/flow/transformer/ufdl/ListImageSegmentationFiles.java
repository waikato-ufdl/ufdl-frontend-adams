...
/*
 * ListImageSegmentationFiles.java
...
import adams.data.conversion.UFDLImageSegmentationDatasetFilesToSpreadSheet;
import adams.data.spreadsheet.SpreadSheet;
import com.github.waikatoufdl.ufdl4j.action.Datasets.Dataset;

/**
 * Lists the images in the incoming image segmentation dataset.
 *
 * @author FracPete (fracpete at waikato dot ac dot nz)
 */
public class ListImageSegmentationFiles
  ...
  public String globalInfo() {
    return "Lists the images in the incoming image segmentation dataset.\n"
  ...
  public String getAdditionalInformation() {
    return new UFDLImageSegmentationDatasetFilesToSpreadSheet().getAdditionalInformation();
  }
  ...
  protected Object doTransform(Dataset dataset, MessageCollection errors) {
    SpreadSheet						result;
    UFDLImageSegmentationDatasetFilesToSpreadSheet	conv;
    String						msg;

    result = null;

    try {
      conv = new UFDLImageSegmentationDatasetFilesToSpreadSheet();
      conv.setFlowContext(getFlowContext());
      conv.setMetaDataKeys(m_MetaDataKeys);
      conv.setInput(dataset);
      msg = conv.convert();
      if (msg != null)
	errors.add("Failed to list image segmentation files for dataset: " + dataset + "\n" + msg);
      else
        result = (SpreadSheet) conv.getOutput();
    }
    catch (Exception e) {
      errors.add("Failed to list image segmentation files for dataset: " + dataset, e);
    }
