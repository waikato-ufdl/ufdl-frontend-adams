...
/*
 * DeleteImageClassificationDataset.java
...
/**
 * Deletes the image classification dataset either via PK or datasetname.
 *
 * @author FracPete (fracpete at waikato dot ac dot nz)
 */
public class DeleteImageClassificationDataset
  ...
  public String globalInfo() {
    return "Deletes the image classification dataset either via PK or dataset name.";
  }
  ...
  protected Object doTransform(Dataset dataset, MessageCollection errors) {
    ...
    if (isLoggingEnabled())
      getLogger().info("Deleting image classification dataset (hard=" + m_Hard + "): " + dataset);

    try {
      result = getDatasetsAction().delete(dataset, m_Hard);
    }
    catch (Exception e) {
      errors.add("Failed to delete image classification dataset (hard=" + m_Hard + "): " + dataset, e);
    }
